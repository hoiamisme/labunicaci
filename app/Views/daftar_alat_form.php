...
<body>
    ...
    <div style="margin-bottom: 20px; padding: 15px; background-color: #f8f9fa; border-radius: 5px;">
        <form method="GET" action="/inventory/daftar-alat" style="display: flex; gap: 10px; align-items: center;">
            <input type="text" 
                   name="search" 
            ...
            <a href="/inventory/daftar-alat" style="padding: 8px 15px; background-color: #6c757d; color: white; text-decoration: none; border-radius: 4px;">
                🗑️ Reset
            </a>
        </form>
    </div>
        ...
        <tbody>
            <?php if (!empty($items)): ?>
                <?php foreach ($items as $item): ?>
                    <tr>
                        <td style="padding: 8px; text-align: center;"><?= $item['id_alat'] ?></td>
                        <td style="padding: 8px;"><?= esc($item['nama_alat']) ?></td>
                        ...
                        <?php if (session()->get('role') === 'admin'): ?>
                            <td style="padding: 8px; text-align: center;">
                                <button onclick="hapusAlat(<?= $item['id_alat'] ?>, '<?= esc($item['nama_alat']) ?>')" 
    ...
    <?php if (($totalPages ?? 1) > 1): ?>
        <div style="margin-top: 20px; text-align: center;">
            ...
            <?php if ($currentPage > 1): ?>
                <a href="/inventory/daftar-alat?page=<?= $currentPage - 1 ?><?= $queryString ?>" 
